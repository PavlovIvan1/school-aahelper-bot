...
# Прокси берётся из .env (config.py читает TELEGRAM_PROXY_URL сам).
# Здесь оставляем возможность переопределить через окружение, но по умолчанию
# ничего не экспортируем, чтобы не перекрывать значение из .env.
